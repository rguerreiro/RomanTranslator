function RomanTranslator() {
    this.roman_numbers = {
        I: { value: 1, max_times: 3, regexp: /I/gi, before: 'X' },
        V: { value: 5, max_times: 1, regexp: /V/gi, before: null },
        X: { value: 10, max_times: 3, regexp: /X/gi, before: 'C' },
        L: { value: 50, max_times: 1, regexp: /L/gi, before: null },
        C: { value: 100, max_times: 3, regexp: /C/gi, before: 'M' },
    ...
    for (var i = 0; i < roman.length; i++) {
        ...
        if (roman_number && roman_number.value) {
            ...
            if (previousRoman && roman_number.value > previousRoman.value) {
                if (previousRoman.before === char) {
                    decimal += (roman_number.value - (2 * previousRoman.value));
                } else {
                    throw new Error('Misplaced character \'' + char + '\' in position ' + (i+1));
                }
            } else {
